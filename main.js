...
let cart = [];
let buttonsDOM = [];
//3 adet class tanımlayalım.gerekli metotları class içinde yazacağız.
//1-ürünleri aldığımız api ile iletişim kurduğumuz yer
//api da oluşturduğumuz products verilerini burada çekeceğiz
class Products {
  async getProducts() {
    try {
      let result = await fetch(
        "https://65cf20ccbdb50d5e5f5a8aa9.mockapi.io/products"
      );
...
//2-UI kısmına verileri aktarıldığı yer
class UI {
...
//3-Local Storage a kaydettiğimiz yer
class Storage {
...
//sayfa yüklendiğinde gerçekleşmesi gereken olayları dinlediğimiz listener oluştururuz.
